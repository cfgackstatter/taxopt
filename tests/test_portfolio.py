...
def test_sell_closes_lot(prices, us_policy):
    ...
    new_p, _ = p.apply_actions(
        [LotClose(lot_index=0, asset="AAPL", quantity=10.0, lot_ref=lot)],
        prices, us_policy, date(2024, 6, 1),
    )
    assert _all_lots(new_p, "AAPL") == []
...
def test_partial_sell_leaves_correct_quantity(prices, us_policy):
    ...
    lot = p.lots["AAPL"][0]
    new_p, _ = p.apply_actions(
        [LotClose(lot_index=0, asset="AAPL", quantity=4.0, lot_ref=lot)],
        prices, us_policy, date(2024, 6, 1),
    )
...
# ---------------------------------------------------------------------------
# Portfolio value and invariants
# ---------------------------------------------------------------------------
...
def test_short_cover_conserves_nav(prices):
    ...
    assert abs(p2.total_value(prices) - p.total_value(prices)) < 1e-6
...
def test_combined_actions_conserve_nav(prices):
    """Long open + short open + lot close all in one apply_actions call."""
    ...
    p3, _ = p2.apply_actions([
        LotClose(lot_index=0, asset="AAPL", quantity=50.0, lot_ref=lot),
        LongOpen("MSFT", 20.0),
        ShortOpen("GOOG", 30.0),
    ], prices, NoTaxPolicy(), date(2024, 1, 1))
...
def test_lot_close_closes_correct_lot(prices, us_policy):
    """LotClose must close exactly lot_ref, not re-sort by lot_method."""
    today = date.today()
    p = Portfolio(cash=0.0)
    lot_a = TaxLot("AAPL", 10.0,  50.0, today - timedelta(days=400))  # LT gain
    lot_b = TaxLot("AAPL", 10.0, 160.0, today - timedelta(days=30))   # ST loss
    p.add_lot(lot_a)
    p.add_lot(lot_b)
    new_p, report = p.apply_actions(
        [LotClose(lot_index=1, asset="AAPL", quantity=10.0, lot_ref=lot_b)],
        prices, us_policy, today,
    )
    assert len(new_p.lots["AAPL"]) == 1
    assert new_p.lots["AAPL"][0].cost_basis == 50.0   # lot_a remains
    assert report.events[0].gain_type == "short_term"  # lot_b was a ST loss
